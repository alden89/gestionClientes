...
class CreateDatabase(object):
  def __init__(self, conexion, cursor):		
	...
	cursor.execute(sql)		
	sql = """CREATE TABLE IF NOT EXISTS tipospago( tipopago INTEGER , nombrepago VARCHAR DEFAULT NULL, PRIMARY KEY(tipopago, nombrepago))"""
	cursor.execute(sql)
	sql="INSERT OR IGNORE INTO usuarios VALUES('%s', '%s')"
	cursor.execute(sql % ('admin', md5.new('admin').hexdigest()))
	conexion.commit()
	sql="""INSERT OR IGNORE INTO tipospago VALUES(1,'Diario'), (2,'Semanal'), (3, 'Mensual')"""
	cursor.execute(sql)
	conexion.commit()		
